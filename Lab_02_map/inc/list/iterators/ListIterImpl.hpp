...
template <typename T> ListIterator<T>::ListIterator(const List<T> &map) {
...
template <typename T> ListIterator<T>::operator bool() const {
  return isValid();
}

template <typename T> auto ListIterator<T>::operator*() const -> reference {
  validatePtr(__LINE__);
  return nodePtr.lock().get()->getValueRef();
}

template <typename T> auto ListIterator<T>::operator->() const -> pointer {
  validatePtr(__LINE__);
  return &(nodePtr.lock().get()->getValueRef());
}
...
template <typename T> bool ListIterator<T>::isValid() const {
  return nodePtr.lock() != nullptr && !nodePtr.expired();
}

template <typename T> void ListIterator<T>::validatePtr(int line) const {
  if (!isValid())
    throw InvalidIterator(__FILE_NAME__, typeid(*this).name(), __FUNCTION__,
                          line);
}

template <typename T> std::shared_ptr<ListNode<T>> ListIterator<T>::getPtr() {
  return nodePtr.lock();
}
