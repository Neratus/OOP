...
template <typename T> ConstListIterator<T>::ConstListIterator() {}
...
ConstListIterator<T>::ConstListIterator(
    const std::shared_ptr<ListNode<T>> &node) {
  nodePtr = node;
}

template <typename T> ConstListIterator<T>::operator bool() const {
  return isValid();
}
...
template <typename T> auto ConstListIterator<T>::operator->() const -> pointer {
  validatePtr(__LINE__);
  return &(nodePtr.lock().get()->getValueRef());
}

// prefix
template <typename T> ConstListIterator<T> &ConstListIterator<T>::operator++() {
  validatePtr(__LINE__);
...
template <typename T> bool ConstListIterator<T>::isValid() const {
  return nodePtr.lock() != nullptr && !nodePtr.expired();
}

template <typename T> void ConstListIterator<T>::validatePtr(int line) const {
  if (!isValid())
