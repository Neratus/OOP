...
HashMapIterator<K, V>::HashMapIterator(const buckets_iterator &current,
                                       const buckets_iterator &end)
    : HashMapIterator(current, end,
...
template <typename K, typename V> HashMapIterator<K, V>::operator bool() const {
  return isValid();
}
...
template <typename K, typename V>
bool HashMapIterator<K, V>::operator==(
    const HashMapIterator<K, V> &iterator) const {
  return currentBucket == iterator.currentBucket &&
         endBucket == iterator.endBucket &&
         elementIterator == iterator.elementIterator;
}

template <typename K, typename V> bool HashMapIterator<K, V>::isValid() const {
...
void HashMapIterator<K, V>::validateIterator(int line) const {
  if (!isValid())
    throw InvalidIterator(__FILE_NAME__, typeid(*this).name(), __FUNCTION__,
                          line);
}

template <typename K, typename V> void HashMapIterator<K, V>::moveNextBucket() {
  while (currentBucket != endBucket &&
         elementIterator == currentBucket->end()) {
    ++currentBucket;
