...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
HashMap<K, V, Hash, KeyEqual>::HashMap() : HashMap(initialSize) {}

template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
HashMap<K, V, Hash, KeyEqual>::HashMap(size_t size)
    : hashFunction(Hash()), keyEqualFunction(KeyEqual()) {
  buckets = List<List<value_type>>(size);
}

template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
HashMap<K, V, Hash, KeyEqual>::HashMap(std::initializer_list<value_type> list)
...
HashMap<K, V, Hash, KeyEqual>::operator=(const hashmap &map) {
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
void HashMap<K, V, Hash, KeyEqual>::insert_or_assign(const K &key,
                                                     const V &value) {
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::insert(const K &key, const V &value)
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::insert(const value_type &entry)
    -> std::pair<iterator, bool> {
...
bool HashMap<K, V, Hash, KeyEqual>::erase(const K &key) noexcept {
  ...
  auto it = std::ranges::find_if(
      begin(index), end(index), [&key, this](const value_type &value) {
        return this->keyEqualFunction(value.first, key);
      });
  if (it == end(index))
    return false;
...
V &HashMap<K, V, Hash, KeyEqual>::at(const K &key) {
  auto it = find(key);
  if (it == end())
    throw InvalidKeyException(__FILE_NAME__, typeid(*this).name(),
...
bool HashMap<K, V, Hash, KeyEqual>::contains(const K &key) {
  ...
  return std::ranges::any_of(begin(index), end(index),
                             [this, &key](const value_type &value) {
                               return this->keyEqualFunction(value.first, key);
                             });
}
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::begin(size_type bucket) const
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::end(size_type bucket) const
    -> const_local_iterator {
  validateBucketIndex(bucket, __LINE__);
  return buckets[bucket].end();
}
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::cbegin(size_type bucket) const
    -> const_local_iterator {
  validateBucketIndex(bucket, __LINE__);
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::cend(size_type bucket) const
    -> const_local_iterator {
  validateBucketIndex(bucket, __LINE__);
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::getBucketSize(size_type bucket) const
    -> List<value_type>::size_type {
  validateBucketIndex(bucket, __LINE__);
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::getBucket(const K &key) const noexcept
    -> size_type {
...
void HashMap<K, V, Hash, KeyEqual>::rehash(size_type count) {
  size_type nextSize = getNextPrime(
      std::max(static_cast<size_type>(getSize() / getMaxLoadFactor()), count));
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
void HashMap<K, V, Hash, KeyEqual>::validateBucketIndex(size_type bucket,
                                                        int line) {
  if (bucket >= getBucketCount())
    throw InvalidBucketIndexException(__FILE_NAME__, typeid(*this).name(),
                                      __FUNCTION__, line);
}
...
template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::insert(List<List<value_type>> &buckets,
  ...
  auto coll = find_if(bucket.begin(), bucket.end(),
                      [&entry, this](const value_type &value) {
                        return this->keyEqualFunction(value.first, entry.first);
                      });
  if (coll != bucket.end())
    return std::make_pair(
        iterator(buckets.begin() + index, buckets.end(), coll), false);

  auto res = bucket.pushFront(entry);
  return std::make_pair(iterator(buckets.begin() + index, buckets.end(), res),
                        true);
}

template <MoveAndCopy K, MoveAndCopy V, HashFunction<K> Hash,
          EqualFunction<K> KeyEqual>
auto HashMap<K, V, Hash, KeyEqual>::getNextPrime(size_type size) const
    -> size_type {
