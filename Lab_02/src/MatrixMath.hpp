...
template <CopyNMoveable T> T Matrix<T>::determinant() const {
  if (isSquare() == false) {
...
template <CopyNMoveable T> Matrix<T> Matrix<T>::transpose() {
...
template <CopyNMoveable T> Matrix<T> &Matrix<T>::neg() {
  *this = operator-();

  return *this;
}

template <CopyNMoveable T> Matrix<T> &Matrix<T>::abs() {
  for (T &elem : *this) {
    if (elem < 0) {
      elem = -elem;
    }
...
template <CopyNMoveable T> Matrix<T> Matrix<T>::inverse() {
...
template <CopyNMoveable T> Matrix<T> &Matrix<T>::dot(const Matrix<T> &matrix) {
  if (this->_equalSize(matrix) == false) {
    throw SizeError(matrix.getColumns(), matrix.getRows(), __FILE__, __LINE__,
                    "Sizes don't match!");
  }
...
Matrix<T> &Matrix<T>::dot(const Matrix<T1> &matrix) {
  if (this->_equalSize(matrix) == false) {
    throw SizeError(matrix.getColumns(), matrix.getRows(), __FILE__, __LINE__,
                    "Sizes don't match!");
  }
...
template <CopyNMoveable T> Matrix<T> &Matrix<T>::divElem(const T &val) {
  *this = operator/(val);
...
Matrix<T> &Matrix<T>::addMatrix(const Matrix<T1> &matrix) {
  *this = operator+(matrix);
...
Matrix<T> &Matrix<T>::addElem(const T1 &val) noexcept {
  *this = operator+(val);
...
template <typename T1>
  requires MatrixArithmetic<T1, T>
Matrix<T> &Matrix<T>::subMatrix(const Matrix<T1> &matrix) {
  *this = operator-(matrix);
...
template <typename T1>
  requires ElementArithmetic<T1, T>
auto Matrix<T>::subElem(const T1 &val) noexcept {
  *this = operator-(val);
...
template <typename T1>
  requires MatrixArithmetic<T1, T>
Matrix<T> &Matrix<T>::mulMatrix(const Matrix<T1> &matrix) {
  *this = operator*(matrix);
...
template <typename T1>
  requires ElementArithmetic<T1, T>
Matrix<T> &Matrix<T>::mulElem(const T1 &val) noexcept {
  *this = (*this) * val;
...
template <typename T1>
  requires MatrixArithmetic<T1, T>
Matrix<T> &Matrix<T>::divMatrix(const Matrix<T1> &matrix) {
  *this = operator/(matrix);
...
template <typename T1>
  requires ElementArithmetic<T1, T>
Matrix<T> &Matrix<T>::divElem(const T1 &val) {
  *this = (*this) / val;
  return *this;
}