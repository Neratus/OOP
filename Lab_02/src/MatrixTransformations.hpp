...
void Matrix<T>::resize(const size_t new_row, const size_t new_col,
                       const T &fill_value) {
...
template <CopyNMoveable T> void Matrix<T>::rotate_right() {
  this->transpose();
  this->vertical_mirror();
}

template <CopyNMoveable T> void Matrix<T>::rotate_left() {
  this->transpose();
...
template <CopyNMoveable T> void Matrix<T>::deleteRow(const size_t ind) {
  if (ind >= nrows) {
...
template <CopyNMoveable T> void Matrix<T>::deleteCol(const size_t ind) {
  if (ind >= ncols) {
    throw IndexOutOfRange(ind, __FILE__, __LINE__, "Can't delete!");
  }
...
template <CopyNMoveable T> void Matrix<T>::vertical_mirror() {
...
template <CopyNMoveable T> void Matrix<T>::horizontal_mirror() {
