...
Matrix<T>::Matrix(size_t rows, size_t cols, const T &fill_value)
...
template <CopyNMoveable T>
Matrix<T>::Matrix(T **cdata, const size_t rows, const size_t cols)
    : BaseMatrix(rows, cols) {
...
Matrix<T>::Matrix(const Matrix<T> &matrix)
    : BaseMatrix(matrix.nrows, matrix.ncols) {
...
template <CopyNMoveable T> Matrix<T>::~Matrix() {
  if (bool(*this)) {
    if (data.use_count() == 1) {
      _cleanRows();
    }
...
template <CopyNMoveable T> Matrix<T>::operator bool() const {
  return nrows && ncols && (data != nullptr);
}

template <CopyNMoveable T> Iterator<T> Matrix<T>::begin() {
  return Iterator<T>(*this, 0, 0);
}

template <CopyNMoveable T> Iterator<T> Matrix<T>::end() {
  return ++Iterator<T>(*this, nrows - 1, ncols - 1);
}

template <CopyNMoveable T> ConstIterator<T> Matrix<T>::begin() const {
  return ConstIterator<T>(*this, 0, 0);
}

template <CopyNMoveable T> ConstIterator<T> Matrix<T>::end() const {
  return ++ConstIterator<T>(*this, nrows - 1, ncols - 1);
}

template <CopyNMoveable T> ConstIterator<T> Matrix<T>::cbegin() const {
  return ConstIterator<T>(*this, 0, 0);
}

template <CopyNMoveable T> ConstIterator<T> Matrix<T>::cend() const {
  return ++ConstIterator<T>(*this, nrows - 1, ncols - 1);
}

template <CopyNMoveable T>
void Matrix<T>::fill(Iterator<T> &start, const Iterator<T> &end,
                     T &fill_value) {
