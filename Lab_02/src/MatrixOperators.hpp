...
template <CopyNMoveable T>
const typename Matrix<T>::MatrixRow &
Matrix<T>::operator[](const size_t ind) const {
  if (this->_isMatrixIndValid(ind) == false) {
    throw IndexOutOfRange(ind, __FILE__, __LINE__);
  }
...
template <CopyNMoveable T> Matrix<T> Matrix<T>::operator/(const T &val) const {
  if (val == 0) {
...
template <CopyNMoveable T> Matrix<T> &Matrix<T>::operator/=(const T &val) {
  *this = operator/(val);
...
template <CopyNMoveable T> Matrix<T> Matrix<T>::operator-() const {
  Matrix<T> tmp(*this);
...
auto Matrix<T>::operator+(const Matrix<T1> &matrix) const {
  if (this->_equalSize(matrix) == false) {
    throw SizeError(matrix.getColumns(), matrix.getRows(), __FILE__, __LINE__,
                    "Sizes don't match!");
  }
...
auto Matrix<T>::operator+(const T1 &val) const noexcept {
...
template <typename T1>
  requires ElementArithmetic<T1, T>
auto Matrix<T>::operator/(const T1 &val) const {
  if (val == 0) {
    throw ZeroDivMatrix(0, 0, __FILE__, __LINE__);
  }
...
template <typename T1>
  requires MatrixArithmetic<T1, T>
Matrix<T> &Matrix<T>::operator-=(const Matrix<T1> &matrix) {
  *this = operator-(matrix);
...
template <typename T1>
  requires ElementArithmetic<T1, T>
Matrix<T> &Matrix<T>::operator-=(const T1 &val) noexcept {
  *this = operator-(val);
...
template <typename T1>
  requires ElementArithmetic<T1, T>
Matrix<T> &Matrix<T>::operator/=(const T1 &val) {
  *this = operator/(val);
...
template <typename T1>
  requires MatrixArithmetic<T1, T>
Matrix<T> &Matrix<T>::operator=(Matrix<T1> &&matrix) noexcept {
  nrows = matrix.getRows();
  ncols = matrix.getColumns();
  this->data = _allocateMatrix(nrows, ncols);
