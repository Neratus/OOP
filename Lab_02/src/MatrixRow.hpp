...
Matrix<T>::MatrixRow::MatrixRow(const std::initializer_list<T> &row) {
...
template <CopyNMoveable T> void Matrix<T>::MatrixRow::_checkPointer(T *cptr) {
  if (!cptr) {
...
template <CopyNMoveable T>
Matrix<T>::MatrixRow &
Matrix<T>::MatrixRow::operator=(const std::initializer_list<T> &values) {
...
template <CopyNMoveable T>
Matrix<T>::MatrixRow &
Matrix<T>::MatrixRow::operator=(const Matrix<T>::MatrixRow &row) {
  SharedPtr<T[]> newrow = _allocateRow(row.getLength());
  this->length = row.getLength();
...
template <CopyNMoveable T>
Matrix<T>::MatrixRow &
Matrix<T>::MatrixRow::operator=(MatrixRow &&row) noexcept {
...
template <CopyNMoveable T> Matrix<T>::MatrixRow::~MatrixRow() {
  this->data.reset();
  this->length = 0;
}

template <CopyNMoveable T> void Matrix<T>::MatrixRow::reset() {
  this->data.reset();
  this->length = 0;
}
...
Matrix<T>::MatrixRow &Matrix<T>::MatrixRow::operator+=(const MatrixRow &row) {
  if (_canAdd(row) == false) {
    throw SizeError(length, row.length, __FILE__, __LINE__,
...
template <CopyNMoveable T>
Matrix<T>::MatrixRow
Matrix<T>::MatrixRow::operator*(const T &val) const noexcept {
  MatrixRow temp = *this;
...
template <CopyNMoveable T>
Matrix<T>::MatrixRow
Matrix<T>::MatrixRow::operator+(const MatrixRow &row) const {
  if (_canAdd(row) == false) {
    throw SizeError(length, row.length, __FILE__, __LINE__,
                    "Rows have different size!");
  }
