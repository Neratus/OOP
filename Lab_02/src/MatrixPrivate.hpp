...
SharedPtr<typename Matrix<T>::MatrixRow[]>
Matrix<T>::_allocateMatrix(const size_t rows, const size_t cols) {
...
bool Matrix<T>::_equalSize(const Matrix<T> &matr) const {
  ...
  if (this->getColumns() == matr.getColumns() &&
      this->getRows() == matr.getRows()) {
    ans = true;
  }
...
bool Matrix<T>::_equalSize(const Matrix<T1> &matr) const {
  bool ans = false;
...
template <CopyNMoveable T>
T Matrix<T>::_MultiplyRowByColumn(const Matrix<T> &matrix, const size_t row,
...
template <CopyNMoveable T>
template <typename T1>
bool Matrix<T>::_CanMultiplyMatrices(const Matrix<T1> &matr) const {
  bool ans = false;

  if (matr.getColumns() == 0 || matr.getRows() == 0 || ncols == 0 ||
      nrows == 0) {
    ans = false;
  } else if (this->getColumns() == matr.getRows()) {
    ans = true;
...
template <CopyNMoveable T> void Matrix<T>::_cleanRows() {
