...
void ModelBuilder::build() { _model = std::make_shared<ModelStructure>(); }
...
void ModelBuilder::buildLink(const std::size_t vertex1Index,
                             const std::size_t vertex2Index) {
  if (!isBuild()) {
  }
...
bool ModelBuilder::isBuild() const { return nullptr != _model; }
