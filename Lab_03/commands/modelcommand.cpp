...
MoveModel::MoveModel(const double dx, const double dy, const double dz,
                     const std::size_t id)
...
ScaleModel::ScaleModel(const double kx, const double ky, const double kz,
                       const std::size_t id)
...
RotateModel::RotateModel(const double ox, const double oy, const double oz,
                         const std::size_t id)
...
void TransformModel::execute() {
  ...
  auto iter = scene->getObject(_id);
  auto model = *iter;
  TransformManagerCreator().createManager()->transformObject(model, _move,
                                                             _scale, _rotate);
}
...
void LoadModel::execute() {
  auto moderator = ModelLoadModeratorCreator().createModerator(_fileName);
...
GroupModelsCommand::GroupModelsCommand(const std::vector<std::size_t> &ids,
                                       std::shared_ptr<std::size_t> id)
    : _ids(ids), _id(id) {}

void GroupModelsCommand::execute() {
  auto scene = SceneManagerCreator().createManager()->getScene();

  auto group = std::make_shared<Composite>();

  for (const auto &id : _ids) {
    Iterator it = scene->getObject(id);
    if (it != scene->end()) {
      group->add(*it);
    }
  }
