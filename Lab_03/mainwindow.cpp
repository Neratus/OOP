...
void MainWindow::on_groupModelsBtn_clicked() {
  QVector<std::pair<std::size_t, QString>> modelItems;
  for (int i = 0; i < ui->modelsCB->count(); ++i) {
    QVariant data = ui->modelsCB->itemData(i, Qt::UserRole);
    QString text = ui->modelsCB->itemText(i);
    qDebug() << "model item" << i << ":" << text
             << ", id =" << data.toULongLong(); // ← сюда
    if (data.isValid())
      modelItems.push_back({data.toULongLong(), text});
  }

  GroupModelsDialog dialog(modelItems, this);
  if (dialog.exec() == QDialog::Accepted) {
    auto selected = dialog.selectedModelIds();

    if (selected.size() < 2) {
      QMessageBox::warning(this, "Ошибка",
                           "Выберите хотя бы два объекта для группировки.");
      return;
    }

    auto newId = std::make_shared<size_t>(0);
    GroupModelsCommand groupCmd(selected, newId);

    try {
      _facade->execute(groupCmd);
    } catch (const BaseException &err) {
      QMessageBox::critical(this, "Ошибка", "Не удалось сгруппировать модели.");
      return;
    }

    QString groupName = QString("Группа моделей №%1").arg(*newId);
    ui->modelsCB->addItem(groupName, static_cast<qulonglong>(*newId));
    _models.push_back(*newId);
...
void MainWindow::on_loadModelBtn_clicked() {
  ...
  auto file = QFileDialog::getOpenFileName(nullptr, "Загрузка модели",
                                           "../lab_03/data");
  ...
  updateScene();
  ui->modelsCB->addItem(QFileInfo(file).fileName(),
...
void MainWindow::on_deleteCameraBtn_clicked() {
  ...
  try {
    ...
    QMessageBox::critical(nullptr, "Ошибка",
                          "Это последняя камера! Для удаления удалите модели!");
    return;
  }
...
void MainWindow::on_moveBtn_clicked() {
  ...
  MoveModel cmd(ui->dxDSB->value(), ui->dyDSB->value(), ui->dzDSB->value(),
...
void MainWindow::on_scaleBtn_clicked() {
  ...
  ScaleModel cmd(ui->kxDSB->value(), ui->kyDSB->value(), ui->kzDSB->value(),
                 _models.at(ui->modelsCB->currentIndex()));
...
void MainWindow::on_rotateBtn_clicked() {
  ...
  RotateModel cmd(ui->oxDSB->value(), ui->oyDSB->value(), ui->ozDSB->value(),
                  _models.at(ui->modelsCB->currentIndex()));
