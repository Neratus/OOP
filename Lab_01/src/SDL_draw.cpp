...
static void init_SDL_color_t(SDL_color_t &color, uint8_t r, uint8_t g,
                             uint8_t b, uint8_t a) {
  color.r = r;
  color.g = g;
  color.b = b;
...
static SDL_color_t get_color(SDL_color color_name) {
  SDL_color_t color;
  switch (color_name) {
  case SDL_WHITE:
...
void clear_canvas(SDL_Renderer *renderer) { SDL_RenderClear(renderer); }
